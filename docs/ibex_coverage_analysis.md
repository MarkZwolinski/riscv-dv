...
branch) directly is not meaningful — the denominators and counting units differ.
...
infrastructure no instruction-only test can reach. Extending VeeR's test set with
interrupt and CSR tests, and scoping coverage to the core pipeline only, would produce
